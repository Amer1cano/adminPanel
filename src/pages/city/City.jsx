...
import { useGetCityDetailsQuery } from "../../services/adminApi";
...
const City = () => {
    ...
    const { data, error, isLoading, isSuccess, refetch } = useGetCityDetailsQuery();
                ...
                <>
                    {addModal && (
                        <div className="w-full h-screen flex items-center justify-center top-0 left-0 inset-0 fixed z-10 bg-[#0000004D]">
                            <AddCity setAddModal={setAddModal} updateBrands={updateBrands} />
                        </div>
                    )}
                    <table className="w-full rounded-lg overflow-hidden shadow-lg">
                        <thead>
                            <tr>
                                ...
                                <th className="py-3 px-4 bg-blue-500 text-white text-left">Text</th>
                                <th className="py-3 px-4 bg-blue-500 text-white text-left">Images</th>
                        ...
                        <tbody>
                            {currentPageData?.map((item, index) => (
                                <CardCity key={item.id} item={item} index={index} updateBrands={updateBrands} />
                            ))}
                        </tbody>
                    </table>
