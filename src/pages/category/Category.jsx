...
import { useGetCategoryDetailsQuery } from "../../services/adminApi";
...
const Category = () => {
    ...
    const { data, error, isLoading, isSuccess, refetch } = useGetCategoryDetailsQuery();
                ...
                <>
                    {addModal && (
                        <div className="w-full h-screen flex items-center justify-center top-0 left-0 inset-0 fixed z-10 bg-[#0000004D]">
                            <AddBrand setAddModal={setAddModal} updateBrands={updateBrands} />
                        </div>
                    )}
                    <table className="w-full">
                        <thead>
                            <tr>
                                <th className="py-3 px-4 bg-blue-400 text-white text-left">Id</th>
                                <th className="py-3 px-4 bg-blue-400 text-white text-left">Name_en</th>
                                <th className="py-3 px-4 bg-blue-400 text-white text-left">Name_ru</th>
                        ...
                        <tbody>
                            {currentPageData?.map((item, index) => (
                                <CardCategory key={item.id} item={item} index={index} updateBrands={updateBrands} />
                            ))}
                        </tbody>
                    </table>
