...
import { useGetBrandsDetailsQuery } from "../../services/adminApi";
...
const Brands = () => {
    ...
    const { data, error, isLoading, isSuccess, refetch } = useGetBrandsDetailsQuery();
                ...
                <>
                    {addModal && (
                        <div className="w-full h-screen flex items-center justify-center top-0 left-0 inset-0 fixed z-10 bg-[#0000004D]">
                            <AddBrands setAddModal={setAddModal} updateBrands={updateBrands} />
                        </div>
                    )}
                    <table className="w-full rounded-xl">
                        <thead>
                            <tr>
                                ...
                                <th className="py-3 px-4 bg-blue-400 text-white text-left">Image</th>
                        ...
                        <tbody>
                            {currentPageData?.map((item, index) => (
                                <CardBrand key={item?.id} item={item} index={index} updateBrands={updateBrands} />
                            ))}
                        </tbody>
                    </table>
